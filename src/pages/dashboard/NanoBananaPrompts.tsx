...
function getImages(item: NanoBananaPrompt): string[] {
  return toStrArr(item.sourceMedia);
}
        ...
        <div className="flex items-start gap-4 px-6 py-5 border-b border-border/40 shrink-0">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-2">
              <span className="text-[10px] font-bold uppercase tracking-wider bg-yellow-500/15 text-yellow-400 px-2 py-0.5 rounded-full">Nano Banana</span>
        ...
        <div className="flex-1 overflow-y-auto custom-scroll p-6 flex flex-col gap-5">
          <div>
            ...
            <button onClick={copy}
              className={cn('mt-3 w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-semibold transition-all',
                copied ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-yellow-500/15 text-yellow-400 border border-yellow-500/30 hover:bg-yellow-500/25')}>
          ...
          {item.author && (item.author.name || item.author.link) && (
            <div className="flex items-center gap-3 p-3 rounded-xl border border-border/40 bg-secondary/20">
              ...
              <p className="text-[12px] font-medium text-foreground flex-1 min-w-0 truncate">{item.author.name}</p>
              {item.author.link && (
                <a href={item.author.link} target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-foreground transition-colors">
    ...
    <div onClick={onClick}
      ...
      <div className="relative w-full bg-secondary/30" style={{ aspectRatio: '16/9' }}>
        {thumb ? (
          ...
          <div className="w-full h-full flex items-center justify-center"><Banana className="h-8 w-8 text-yellow-400/20" /></div>
        )}
        <div className="absolute top-2 left-2">
          <span className="bg-black/70 backdrop-blur-sm text-[10px] font-bold text-yellow-400 px-2 py-0.5 rounded-full">NanoBanana</span>
        </div>
        {images.length > 1 && (
        ...
        <div className="flex items-center justify-between mt-auto pt-2 border-t border-white/5">
          ...
          <button
            ...
            className={cn(
              ...
              copied ? 'bg-green-500/15 text-green-400' : 'bg-white/[0.05] text-muted-foreground hover:bg-yellow-500/15 hover:text-yellow-400',
            )}
          >
...
export default function NanoBananaPrompts() {
  ...
  const filtered = useMemo(() => {
    ...
    return sorted;
  }, [items, search, sort, mediaOnly, category]);
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <PageHeader icon={Banana} title="Nano Banana Prompts"
        description="12,000+ curated image generation prompts with reference media."
        count={items.length} iconColor="text-yellow-400" />
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        ...
        <button onClick={() => setMediaOnly(v => !v)}
          className={cn('flex items-center gap-1.5 h-10 px-3.5 rounded-xl border text-[12px] font-medium transition-all',
            mediaOnly ? 'bg-yellow-500/15 border-yellow-500/35 text-yellow-400' : 'bg-white/[0.03] border-border/35 text-muted-foreground hover:text-foreground hover:border-border/50')}>
